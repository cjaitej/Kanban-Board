...
export default function DetailedCard({
  ...
  const [title, setTitle] = useState(id_data["title"]);
  ...
  function handleSave() {
    ...
    try {
      newdueDate = `${String(dueDate.getMonth() + 1).padStart(2, "0")}/${String(
        dueDate.getDate()
  ...
  useEffect(() => {
    fetch(`http://localhost:5000/get_data/${id}`)
      ...
      .then((data) => {
        ...
        setstage(data["segment"]);
        setSelectedPriority(data["level"]);
        setsublist(data["subtask"]);
  ...
  return (
    <div className="createtask-full-screen-blur">
      <div className="createtask-body">
        <div className="createtask-heading">Task - {id}</div>
                ...
                {sublist.map((item, index) => (
                  ...
                  >
                    <div className="inside-sublist">
                      <div className="subtask-done">{item}</div>
        ...
        <div className="createtask-button">
          <div onClick={handleSave} className="createtask-button-create">
            Save
          </div>
